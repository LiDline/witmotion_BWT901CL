...
def graph_settings():
    ...
    # Выводим глобальные макс значения:
    # fig.add_annotation(dict(font=dict(color='black',size=15), x=0.86, y=1.02,
    #                 showarrow=False,text=f"Макс лин. уск.:",
    ...
    # fig.update_layout(xaxis=dict(domain=[0, 0.85]),
    #             xaxis2=dict(domain=[0, 0.85]),
    #             xaxis3=dict(domain=[0, 0.85]),
    #             )  

    fig.update_layout(           #Позиционирование заголовка
               title={
                    'text': f"Получаемые характеристики датчика BWT901CL; дата: {datetime.datetime.now().replace(microsecond=0)}",
